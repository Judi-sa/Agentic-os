...
MIT. See [LICENSE](./LICENSE). Use it, fork it, ship it commercially, no attribution required.

## Contributing

Pull requests welcome. The system is deliberately small, keep it that way.

**High-value contributions:**
- New `verify.sh` checks for secret formats or client-data patterns not yet covered (the current set targets Stripe, Supabase JWTs, AWS keys, add your own stack)
- Real-world failure patterns worth documenting in the lessons format (`[date] | what went wrong | rule to prevent it`)
- Adaptations of `CLAUDE.md` / `AGENTS.md` for stacks other than Claude Code (issue first, so the constitution's laws stay portable across the change)
- Bug reports on `verify.sh` false positives/negatives, these directly affect trust in the whole loop

**Before opening a PR:**
1. Open an issue first for anything touching a law in `CLAUDE.md` or a role in `AGENTS.md`. These files are meant to change slowly and on purpose, not by accumulation of small PRs.
2. Scripts only: run `./scripts/verify.sh` against your own change if it modifies `verify.sh` or `retro.sh` itself. `bash -n` at minimum for syntax.
3. Keep additions in the same shape as the rest: a law is a `NEVER`, a checkable command, or a fixed format, nothing softer.

**Not in scope:** turning this into a framework with plugins, config DSLs, or a hosted dashboard. It stays four files someone can read in ten minutes and trust.
